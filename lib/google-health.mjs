...
async function reconcileData(accessToken, type, filter, dataSourceFamily = 'users/me/dataSourceFamilies/google-sources') {
  const qs = new URLSearchParams({ dataSourceFamily });
  if (filter) qs.set('filter', filter);
  return healthFetch(accessToken, `https://health.googleapis.com/v4/users/me/dataTypes/${type}/dataPoints:reconcile?${qs}`);
}

async function sleepData(accessToken, filter) {
  // Reconciliation gives us one main sleep record when Fitbit/Google sources overlap.
  // Fall back to list if reconciliation is temporarily unavailable for the account.
  try {
    return await reconcileData(accessToken, 'sleep', filter);
  } catch (error) {
    return listData(accessToken, 'sleep', filter);
  }
}
...
function utcOffsetSeconds(value) {
  if (value == null) return 0;
  const match = String(value).match(/^(-?\d+(?:\.\d+)?)s$/);
  return match ? Number(match[1]) : 0;
}

function localDateFromPhysicalTime(time, utcOffset) {
  const ms = new Date(time).getTime();
  if (!Number.isFinite(ms)) return '';
  return new Date(ms + utcOffsetSeconds(utcOffset) * 1000).toISOString().slice(0, 10);
}

function localMinutesFromPhysicalTime(time, utcOffset) {
  const ms = new Date(time).getTime();
  if (!Number.isFinite(ms)) return null;
  const shifted = new Date(ms + utcOffsetSeconds(utcOffset) * 1000);
  let mins = shifted.getUTCHours() * 60 + shifted.getUTCMinutes();
  if (mins < 12 * 60) mins += 24 * 60;
  return mins;
}

function sleepDateKey(sleep) {
  const civil = sleep?.interval?.civilEndTime?.date;
  return parseCivilDate(civil) || localDateFromPhysicalTime(sleep?.interval?.endTime, sleep?.interval?.endUtcOffset);
}

function sleepBedtimeMinutes(sleep) {
  return civilTimeMinutes(sleep?.interval?.civilStartTime) ?? localMinutesFromPhysicalTime(sleep?.interval?.startTime, sleep?.interval?.startUtcOffset);
}
...
function sleepStageSummary(sleep) {
  const summaryStages = sleep?.summary?.stagesSummary;
  if (Array.isArray(summaryStages) && summaryStages.length) {
    return Object.fromEntries(summaryStages.map(x => [String(x.type || '').toUpperCase(), n(x.minutes)]));
  }
  const totals = {};
  for (const stage of sleep?.stages || []) {
    const type = String(stage?.type || '').toUpperCase();
    if (!type) continue;
    totals[type] = (totals[type] || 0) + intervalMinutes(stage.startTime, stage.endTime);
  }
  return totals;
}

function sleepMetrics(sleep) {
  if (!sleep) return { minutesAsleep: 0, minutesInPeriod: 0, minutesAwake: 0, stages: {} };
  const stages = sleepStageSummary(sleep);
  const periodFallback = intervalMinutes(sleep.interval?.startTime, sleep.interval?.endTime);
  const awakeFallback = n(stages.AWAKE);
  const asleepFromStages = ['LIGHT', 'DEEP', 'REM', 'ASLEEP'].reduce((sum, key) => sum + n(stages[key]), 0);
  const minutesInPeriod = n(sleep.summary?.minutesInSleepPeriod, periodFallback);
  const minutesAwake = n(sleep.summary?.minutesAwake, awakeFallback);
  const minutesAsleep = n(sleep.summary?.minutesAsleep, asleepFromStages || Math.max(0, minutesInPeriod - minutesAwake));
  return { minutesAsleep, minutesInPeriod, minutesAwake, stages };
}

function sleepConsistency(mainSleeps) {
  const bedtimes = mainSleeps.map(sleepBedtimeMinutes).filter(Number.isFinite);
...
export async function buildDashboard(accessToken, targetDate) {
  ...
  const calls = await Promise.allSettled([
    ...
    sleepData(accessToken, `sleep.interval.civil_end_time >= "${start30}T00:00:00" AND sleep.interval.civil_end_time < "${end}T00:00:00"`),
  ...
  const explicitlyMain = sleeps.filter(s => s.metadata?.main === true);
  const mainSleeps = explicitlyMain.length ? explicitlyMain : sleeps.filter(s => s.metadata?.nap !== true);
  const latestSleep = mainSleeps[0] || sleeps[0] || null;
  const sleepByDay = new Map();
  for (const s of mainSleeps) {
    const d = sleepDateKey(s);
    if (!d) continue;
    const existing = sleepByDay.get(d);
    // Prefer the main / longer nightly record if multiple records still survive reconciliation.
    if (!existing || sleepMetrics(s).minutesAsleep > sleepMetrics(existing).minutesAsleep) sleepByDay.set(d, s);
  }
  ...
  const validSleepMinutes = [...sleepByDay.values()].map(s => sleepMetrics(s).minutesAsleep).filter(v => Number.isFinite(v) && v > 0);
  ...
  const latestSleepMetrics = sleepMetrics(latestSleep);
  const sleepMinutes = latestSleepMetrics.minutesAsleep;
  const sleepPeriod = latestSleepMetrics.minutesInPeriod;
  ...
  const stage = latestSleepMetrics.stages;
  const history30 = allDays.map(d => {
    const sl = sleepByDay.get(d);
    const daySleep = sl ? sleepMetrics(sl).minutesAsleep : NaN;
  ...
  return {
    ...
    sleep: latestSleep ? {
      ...
      minutesAwake: latestSleepMetrics.minutesAwake,
      efficiency,
      score: pulseSleepScore,
      stages: { awake: Math.round(stage.AWAKE || 0), rem: Math.round(stage.REM || 0), light: Math.round(stage.LIGHT || 0), deep: Math.round(stage.DEEP || 0) },
      endDate: sleepDateKey(latestSleep)
    } : null,
    training: {
    ...
    trends: {
      ...
      sleepHours: allDays.map(d => {
        const sl = sleepByDay.get(d);
        const v = sl ? sleepMetrics(sl).minutesAsleep : NaN;
        return Number.isFinite(v) && v > 0 ? Number((v / 60).toFixed(1)) : null;
      }),
      recovery: history30.map(x => x.recovery),
      sleepTrackedNights: allDays.filter(d => sleepByDay.has(d)).length,
...
export { overlapMinutes, sleepDateKey, sleepMetrics };
